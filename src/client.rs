...
use crate::headers::l2::create_l2_headers;
use crate::http::HttpClient;
use crate::order_builder::{
    calculate_buy_market_price, calculate_sell_market_price, OrderBuilder,
};
use crate::types::*;
...
impl ClobClient {
    /// Creates a new ClobClient instance
    ///
    /// # Arguments
    ///
    ...
    ///
    /// let client = ClobClient::new("https://clob.polymarket.com".to_string(), Chain::Polygon);
    /// ```
    pub fn new(host: String, chain_id: Chain) -> Self {
        ...
        Self {
            ...
            wallet: None,
            creds: None,
            order_builder: OrderBuilder::new(chain_id),
    ...
    /// Creates a new ClobClient with API credentials
    pub fn with_credentials(
        host: String,
        chain_id: Chain,
        creds: ApiKeyCreds,
    ) -> Self {
        let mut client = Self::new(host, chain_id);
        client.creds = Some(creds);
        client
    }

    /// Adds a wallet to the client (builder pattern)
    pub fn with_wallet(mut self, wallet: LocalWallet) -> Self {
        self.wallet = Some(wallet);
        self
    }

    /// Creates a new ClobClient with wallet and credentials
    pub fn with_credentials_and_wallet(
        host: String,
        chain_id: Chain,
        wallet: LocalWallet,
        creds: ApiKeyCreds,
    ) -> Self {
        Self::new(host, chain_id)
            .with_wallet(wallet)
            .with_credentials_inner(creds)
    }

    /// Sets credentials (internal helper for builder pattern)
    fn with_credentials_inner(mut self, creds: ApiKeyCreds) -> Self {
        self.creds = Some(creds);
        self
    }

    /// Sets the geo-block token
    pub fn with_geo_block_token(mut self, token: String) -> Self {
        self.geo_block_token = Some(token);
        self
    }

    /// Enables using server time for signatures
    ...
    pub async fn get_sampling_markets(&self, next_cursor: Option<String>) -> ClobResult<PaginationPayload> {
        let cursor = next_cursor.unwrap_or_else(|| INITIAL_CURSOR.to_string());
        let endpoint = format!("{}{}", self.host, endpoints::GET_SAMPLING_MARKETS);
    ...
    pub async fn get_simplified_markets(&self, next_cursor: Option<String>) -> ClobResult<PaginationPayload> {
        let cursor = next_cursor.unwrap_or_else(|| INITIAL_CURSOR.to_string());
        let endpoint = format!("{}{}", self.host, endpoints::GET_SIMPLIFIED_MARKETS);
    ...
    pub async fn get_sampling_simplified_markets(&self, next_cursor: Option<String>) -> ClobResult<PaginationPayload> {
        let cursor = next_cursor.unwrap_or_else(|| INITIAL_CURSOR.to_string());
        let endpoint = format!("{}{}", self.host, endpoints::GET_SAMPLING_SIMPLIFIED_MARKETS);
    ...
    pub async fn get_order_book(&self, token_id: &str) -> ClobResult<OrderBookSummary> {
        let endpoint = format!("{}{}", self.host, endpoints::GET_ORDER_BOOK);
    ...
    pub async fn get_order_books(&self, params: Vec<BookParams>) -> ClobResult<Vec<OrderBookSummary>> {
        let endpoint = format!("{}{}", self.host, endpoints::GET_ORDER_BOOKS);
        self.http_client.post(&endpoint, None, Some(params), None).await
    }

    /// Gets midpoint price for a token
    pub async fn get_midpoint(&self, token_id: &str) -> ClobResult<serde_json::Value> {
        let endpoint = format!("{}{}", self.host, endpoints::GET_MIDPOINT);
    ...
    pub async fn get_prices(&self, params: Vec<BookParams>) -> ClobResult<serde_json::Value> {
        let endpoint = format!("{}{}", self.host, endpoints::GET_PRICES);
        self.http_client.post(&endpoint, None, Some(params), None).await
    }

    /// Gets spread for a token
    pub async fn get_spread(&self, token_id: &str) -> ClobResult<serde_json::Value> {
        let endpoint = format!("{}{}", self.host, endpoints::GET_SPREAD);
    ...
    pub async fn get_last_trades_prices(&self, params: Vec<BookParams>) -> ClobResult<serde_json::Value> {
        let endpoint = format!("{}{}", self.host, endpoints::GET_LAST_TRADES_PRICES);
        self.http_client.post(&endpoint, None, Some(params), None).await
    }

    /// Gets historical prices
    pub async fn get_prices_history(&self, params: PriceHistoryFilterParams) -> ClobResult<Vec<MarketPrice>> {
        let endpoint = format!("{}{}", self.host, endpoints::GET_PRICES_HISTORY);
        ...
        self.http_client.get(&endpoint, None, Some(query_params)).await
    }
    ...
    pub async fn get_tick_size(&self, token_id: &str) -> ClobResult<TickSize> {
        ...
        let response: TickSizeResponse = self.http_client.get(&endpoint, None, Some(params)).await?;
        let tick_size = crate::utilities::parse_tick_size(&response.minimum_tick_size)
            .ok_or_else(|| ClobError::Other(format!("Invalid tick size: {}", response.minimum_tick_size)))?;
        
        // Cache the result
        self.tick_sizes.borrow_mut().insert(token_id.to_string(), tick_size);
    ...
    pub async fn get_neg_risk(&self, token_id: &str) -> ClobResult<bool> {
        ...
        // Cache the result
        self.neg_risk.borrow_mut().insert(token_id.to_string(), response.neg_risk);
    ...
    pub async fn get_fee_rate_bps(&self, token_id: &str) -> ClobResult<u32> {
        ...
        // Cache the result
        self.fee_rates.borrow_mut().insert(token_id.to_string(), response.maker_base_fee_rate_bps);
        
        Ok(response.maker_base_fee_rate_bps)
    }

    /// Gets public market trade events
    pub async fn get_market_trades_events(&self, condition_id: &str) -> ClobResult<Vec<MarketTradeEvent>> {
        let endpoint = format!("{}{}{}", self.host, endpoints::GET_MARKET_TRADES_EVENTS, condition_id);
        self.http_client.get(&endpoint, None, None).await
    }
    ...
    pub async fn get_raw_rewards_for_market(&self, condition_id: &str) -> ClobResult<serde_json::Value> {
        let endpoint = format!("{}{}{}", self.host, endpoints::GET_REWARDS_MARKETS, condition_id);
        self.http_client.get(&endpoint, None, None).await
    }
    ...
    pub async fn create_api_key(&self, nonce: Option<u64>) -> ClobResult<ApiKeyCreds> {
        ...
        // Make request
        let endpoint = format!("{}{}", self.host, endpoints::CREATE_API_KEY);
        let response: ApiKeyRaw = self.http_client
            .post(&endpoint, Some(headers), None::<()>, None)
            .await?;
    ...
    pub async fn derive_api_key(&self, nonce: Option<u64>) -> ClobResult<ApiKeyCreds> {
        ...
        // Make request
        let endpoint = format!("{}{}", self.host, endpoints::DERIVE_API_KEY);
        let response: ApiKeyRaw = self.http_client
            .get(&endpoint, Some(headers), None)
            .await?;
    ...
    pub async fn delete_api_key(&self) -> ClobResult<serde_json::Value> {
        ...
        let endpoint = format!("{}{}", self.host, endpoint_path);
        self.http_client.delete(&endpoint, Some(headers), None::<()>, None).await
    }
    ...
    pub async fn post_order(
        ...
        // Make request
        let endpoint = format!("{}{}", self.host, endpoint_path);
        self.http_client.post(&endpoint, Some(headers), Some(order_payload), None).await
    }
    ...
    pub async fn post_orders(&self, orders: Vec<PostOrdersArgs>) -> ClobResult<serde_json::Value> {
        ...
        let owner = &creds.key;
        let payloads: Vec<_> = orders.iter().map(|arg| {
            serde_json::json!({
                "order": arg.order,
                "owner": owner,
        ...
        let endpoint = format!("{}{}", self.host, endpoint_path);
        self.http_client.post(&endpoint, Some(headers), Some(payloads), None).await
    }
    ...
    pub async fn cancel_order(&self, order_id: &str) -> ClobResult<serde_json::Value> {
        ...
        let endpoint = format!("{}{}", self.host, endpoint_path);
        self.http_client.delete(&endpoint, Some(headers), Some(payload), None).await
    }
    ...
    pub async fn cancel_market_orders(&self, params: OrderMarketCancelParams) -> ClobResult<serde_json::Value> {
        self.can_l2_auth()?;
        ...
        let endpoint = format!("{}{}", self.host, endpoint_path);
        self.http_client.delete(&endpoint, Some(headers), Some(params), None).await
    }
    ...
    pub async fn get_open_orders(&self, params: Option<OpenOrderParams>) -> ClobResult<OpenOrdersResponse> {
        self.can_l2_auth()?;
        ...
        let params = if query_params.is_empty() { None } else { Some(query_params) };
    ...
    pub async fn get_trades(&self, params: Option<TradeParams>) -> ClobResult<Vec<Trade>> {
        ...
        while next_cursor != END_CURSOR {
            let response = self.get_trades_paginated(params.clone(), Some(next_cursor.clone())).await?;
    ...
    pub async fn get_trades_paginated(
        ...
        // Add cursor
        query_params.insert("next_cursor".to_string(), cursor.unwrap_or_else(|| INITIAL_CURSOR.to_string()));
        ...
        self.http_client.get(&endpoint, Some(headers), Some(query_params)).await
    }
    ...
    pub async fn are_orders_scoring(&self, params: OrdersScoringParams) -> ClobResult<OrdersScoring> {
        self.can_l2_auth()?;
    ...
    pub async fn get_total_earnings_for_user_for_day(&self, date: &str) -> ClobResult<Vec<TotalUserEarning>> {
        self.can_l2_auth()?;
    ...
    pub async fn create_builder_api_key(&self) -> ClobResult<BuilderApiKey> {
        ...
        let endpoint = format!("{}{}", self.host, endpoint_path);
        self.http_client.post(&endpoint, Some(headers), None::<()>, None).await
    }
    ...
    pub async fn create_order(
        ...
        let signed_order = self.order_builder.build_order(&order, &create_options).await?;
...
mod tests {
    ...
    fn test_client_creation() {
        let client = ClobClient::new("https://clob.polymarket.com".to_string(), Chain::Polygon);
        assert_eq!(client.host, "https://clob.polymarket.com");
        assert_eq!(client.chain_id, Chain::Polygon);
    }
    ...
    #[test]
    fn test_l2_auth_check() {
        let client = ClobClient::new("https://clob.polymarket.com".to_string(), Chain::Polygon);
        assert!(client.can_l2_auth().is_err());
    }
}
